export default {
    ...
    mutations: {
      randomCloudQuestion(state) {
        state.cloudQuestions.sort(() => Math.floor(Math.random() - 0.5));
      },
    },
    actions: {
      cloudQuestionLoop(context) {
        setInterval(() => {
          context.commit("randomCloudQuestion");
        }, 5000);
      },
      randomCloudQuestion(context) {
        context.commit("randomCloudQuestion");
      },
    },
  };
